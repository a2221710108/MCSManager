...
// --- 版本選擇 ---
const selectedVersion = ref<"1.21.x" | "26.x" | "1.12.x">("26.x");
const activeKeys = ref<string[]>([t("環境與時間")]);

// ---------- 1.21.x 指令 ----------
const COMMAND_GROUPS_121: CommandGroup[] = [
  {
    group: t("常用指令"),
    icon: SettingOutlined,
    commands: [
      { label: t("切換難度"), cmd: "difficulty {val}", options: ["peaceful", "easy", "normal", "hard"] },
      { label: t("切換天氣"), cmd: "weather {val}", options: ["clear", "rain", "thunder"] },
      { label: t("設置時間"), cmd: "time set {val}", options: ["day", "night", "noon", "midnight"] },
      { label: t("給予物品"), cmd: "give {player} minecraft:diamond 1" },
      { label: t("切換模式"), cmd: "gamemode {val} {player}", options: ["survival", "creative", "adventure", "spectator"] },
    ]
  },
  {
    group: t("1.21 新特性"),
    ...
    commands: [
      { label: t("調整刻速率"), cmd: "tick rate {text}", placeholder: t("數值") },
      { label: t("生成結構"), cmd: "place structure {text}", placeholder: t("結構名稱") },
    ]
  }
];

// ---------- 26.x 指令 (所有原有指令) ----------
const COMMAND_GROUPS_26: CommandGroup[] = [
  {
    group: t("環境與時間"),
  ...
  },
  {
    ...
    commands: [
      ...
      { label: t("燃燒時間"), cmd: "attribute @a minecraft:burning_time base set {text}", placeholder: t("數值") },
...
// ---------- 1.12.x 指令 ----------
const COMMAND_GROUPS_112: CommandGroup[] = [
  {
    group: t("基本指令"),
    icon: SettingOutlined,
    commands: [
      ...
      { label: t("切換天氣"), cmd: "weather {val}", options: ["clear", "rain", "thunder"] },
      { label: t("設置時間"), cmd: "time set {val}", options: ["day", "night", "noon", "midnight"] },
      { label: t("傳送"), cmd: "tp {player} {player}" },
      { label: t("給予經驗"), cmd: "xp {text} {player}", placeholder: t("經驗值") },
      { label: t("切換遊戲模式"), cmd: "gamemode {val} {player}", options: ["survival", "creative", "adventure", "spectator"] },
    ]
  },
  {
    group: t("1.12 常用"),
    icon: ThunderboltOutlined,
    commands: [
      { label: t("召喚生物"), cmd: "summon {text} ~ ~ ~", placeholder: t("生物ID") },
      { label: t("給予物品"), cmd: "give {player} {text} {text}", placeholder: [t("物品ID"), t("數量")] },
      { label: t("設置重生點"), cmd: "spawnpoint {player}" },
    ]
  }
];
...
const VERSION_COMMANDS: Record<string, CommandGroup[]> = {
  "1.21.x": COMMAND_GROUPS_121,
  "26.x": COMMAND_GROUPS_26,
  "1.12.x": COMMAND_GROUPS_112,
};
