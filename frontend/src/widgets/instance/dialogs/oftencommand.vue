...
const COMMAND_GROUPS_12111: CommandGroup[] = [
  ...
  },
  {
    ...
    commands: [
      ...
      { label: t("地獄堡壘"), cmd: "locate structure fortress" },
  ...
  },
  {
    ...
    commands: [
      ...
      { label: t("燃燒時間"), cmd: "attribute {player} minecraft:burning_time base set {text}", placeholder: t("數值") },
  ...
  },
  {
    ...
    commands: [
      ...
      { label: t("隨機洞穴音效"), cmd: "playsound ambient.cave master {player} ~ ~ ~ {text} {text}", placeholder: [t("玩家"), t("範圍(0.1以上)"), t("音調(0.1以上)")] },
      ...
      { label: t("末影龍死亡"), cmd: "playsound entity.ender_dragon.death master {player} ~ ~ ~ {text} {text}", placeholder: [t("玩家"), t("範圍(0.1以上)"), t("音調(0.1以上)")] },
    ]
  },
  {
    group: t("召喚（敵對生物）"),
  ...
  },
  {
    ...
    commands: [
      ...
      { label: t("失明"), cmd: "effect give {player} blindness {text} {text}", placeholder: [t("玩家"), t("秒數"), t("等級")] },
      { label: t("飢餓"), cmd: "effect give {player} hunger {text} {text}", placeholder: [t("玩家"), t("秒數"), t("等級")] },
      { label: t("虛弱"), cmd: "effect give {player} weakness {text} {text}", placeholder: [t("玩家"), t("秒數"), t("等級")] },
      { label: t("中毒"), cmd: "effect give {player} poison {text} {text}", placeholder: [t("玩家"), t("秒數"), t("等級")] },
      { label: t("凋零"), cmd: "effect give {player} wither {text} {text}", placeholder: [t("玩家"), t("秒數"), t("等級")] },
...
const COMMAND_GROUPS_12110: CommandGroup[] = [
  ...
  },
  {
    ...
    icon: SettingOutlined,
    commands: [
      { label: t("自然回血"), cmd: "gamerule naturalRegeneration {val}", options: ["true", "false"] },
      { label: t("溺水傷害"), cmd: "gamerule drowningDamage {val}", options: ["true", "false"] },
      { label: t("摔落傷害"), cmd: "gamerule fallDamage {val}", options: ["true", "false"] },
      { label: t("火焰傷害"), cmd: "gamerule fireDamage {val}", options: ["true", "false"] },
      { label: t("冰凍傷害"), cmd: "gamerule freezeDamage {val}", options: ["true", "false"] },
      { label: t("原諒死者"), cmd: "gamerule forgiveDeadPlayers {val}", options: ["true", "false"] },
      { label: t("怪物無差別仇恨"), cmd: "gamerule universalAnger {val}", options: ["true", "false"] },
    ]
  },
  {
    group: t("遊戲規則 (上帝)"),
    ...
    commands: [
      ...
      { label: t("鞘翅移動檢測"), cmd: "gamerule disableElytraMovementCheck {val}", options: ["true", "false"] },
      { label: t("積雪厚度"), cmd: "gamerule snowAccumulationHeight {text}", placeholder: t("1 表示一格") },
      { label: t("創造傳送門等待時間"), cmd: "gamerule playersNetherPortalCreativeDelay {text}", placeholder: t("20 代表一秒") },
      { label: t("生存傳送門等待時間"), cmd: "gamerule playersNetherPortalDefaultDelay {text}", placeholder: "80" },
      { label: t("跳過夜晚睡眠比例"), cmd: "gamerule playersSleepingPercentage {text}", placeholder: t("大於100將無法跳過") },
      { label: t("TNT爆炸"), cmd: "gamerule tntExplodes {val}", options: ["true", "false"] },
      { label: t("設定世界中心"), cmd: "worldborder center {text}", placeholder: "x z" },
      { label: t("設定世界邊界"), cmd: "worldborder set {text}", placeholder: "" },
      { label: t("重置世界邊界"), cmd: "worldborder set 59999968" },
    ]
  },
  {
    ...
    icon: SettingOutlined,
    commands: [
      { label: t("生物自然生成"), cmd: "gamerule doMobSpawning {val}", options: ["true", "false"] },
      { label: t("掠奪者巡邏隊"), cmd: "gamerule doPatrolSpawning {val}", options: ["true", "false"] },
      { label: t("夜魅"), cmd: "gamerule doInsomnia {val}", options: ["true", "false"] },
      { label: t("流浪商人"), cmd: "gamerule doTraderSpawning {val}", options: ["true", "false"] },
      { label: t("伏守者"), cmd: "gamerule doWardenSpawning {val}", options: ["true", "false"] },
      { label: t("停用突襲"), cmd: "gamerule disableRaids {val}", options: ["true", "false"] },
    ]
  },
  {
    group: t("播放音效"),
...
// 版本指令對照表
const VERSION_COMMANDS: Record<string, CommandGroup[]> = {
  "1.21.11及以上": COMMAND_GROUPS_12111,
  "1.21.10及以下": COMMAND_GROUPS_12110,
};
...
const filteredGroups = computed(() => {
  ...
  const groups = currentVersionCommands.value;
...
// 自動展開邏輯
watch([searchQuery, selectedVersion], () => {
  const groups = filteredGroups.value;
  if (searchQuery.value.trim()) {
    activeKeys.value = groups.map(g => g.group);
  } else {
    activeKeys.value = groups.length > 0 ? [groups[0].group] : [];
  }
});
...
const fetchPlayers = async () => {
  ...
  try {
    const url = `https://api.mcstatus.io/v2/status/java/${ip}:${port || 25565}?t=${Date.now()}`;
